...
public class Char : MainObject
{
	...
	public static sbyte PK_PHE1;

	public static sbyte PK_PHE2;

	public static sbyte PK_PHE3;
	...
	public static sbyte[] locate;

	private long timeLastCheck;

	public long timelastSendmove;

	public static short delaySendmove;
	...
	public static mHashtable ALL_PART_EFF;
	...
	{
		if (nClass.classId != 2 && nClass.classId != 4)
		{
			return nClass.classId == 6;
		}
		return true;
	}
	...
	{
		isAFocusDie = (isAResuscitate = false);
		myChar = null;
	}
	...
	public virtual void update()
	{
		...
		if (isWolf)
		{
			...
			{
				Domsang obj = (Domsang)vDomsang.elementAt(j);
				obj.update();
				if (obj.frame >= 6)
				{
		...
		else if (isMoto)
		{
			...
			{
				Domsang obj2 = (Domsang)vDomsang.elementAt(k);
				obj2.update();
				if (obj2.frame >= 6)
				{
		...
		else
		{
			...
			{
				Domsang obj3 = (Domsang)vDomsang.elementAt(l);
				obj3.update();
				if (obj3.frame >= 6)
				{
		...
		if (me && isAHP && Code.T == null && cHP < cMaxHP * aHpValue / 100 && mSystem.currentTimeMillis() - lastUseHP > 300 && statusMe != 14 && statusMe != 5 && cHP > 0)
		{
			...
			{
				for (int n = 0; n < vEff.size() && ((Effect)getMyChar().vEff.elementAt(n)).template.type != 17; n++)
				{
					if (n == vEff.size() - 1)
					{
						GameScr.gI().doUseHP();
		...
		if (me && isAFood && Code.T == null && GameCanvas.gameTick % 10 == 0 && !GameScr.isPaintAuto && statusMe != 14 && statusMe != 5 && cHP > 0)
		{
			...
			{
				for (int num2 = 0; num2 < vEff.size() && ((Effect)getMyChar().vEff.elementAt(num2)).template.type != 0; num2++)
				{
					if (num2 != vEff.size() - 1)
					{
						continue;
		...
		if (me && isABuff && Code.T == null && getMyChar().statusMe != 14 && getMyChar().statusMe != 5 && cHP > 0 && mSystem.currentTimeMillis() - lastTimeUseSkill > 500)
		{
			for (int num4 = 0; num4 < vSkill.size(); num4++)
			{
				...
				{
					Effect effect = (Effect)vEff.elementAt(num5);
		...
		if (cHP > 0)
		{
			...
			{
				Effect effect2 = (Effect)vEff.elementAt(num7);
		...
		if (me)
		{
			...
			else if (statusMe != 4 && (Res.abs(cx - cxSend) >= 90 || Res.abs(cy - cySend) >= 90))
			{
				_ = cy;
				_ = cySend;
			}
	...
	private void playSound(SkillPaint skillPaint)
	{
		...
		float volume = (me ? 0.8f : 0.4f);
		switch (nClass.classId)
		{
		case 0:
			Sound.play(Sound.MKiemGo, volume);
			break;
		case 1:
			switch (num)
			{
			default:
				Sound.play(Sound.MKiem3, volume);
				break;
			case 1:
				Sound.play(Sound.MKiem2, volume);
				break;
			case 0:
				Sound.play(Sound.MKiem, volume);
				break;
			}
			break;
		case 2:
			Sound.play(Sound.MTieu, volume);
			break;
		case 3:
			Sound.play(Sound.MKunai, volume);
			break;
		case 4:
			switch (num)
			{
			default:
				Sound.play(Sound.MCung3, volume);
				break;
			case 1:
				Sound.play(Sound.MCung2, volume);
				break;
			case 0:
				Sound.play(Sound.MCung, volume);
				break;
			}
			break;
		case 5:
			if (skillPaint.id == 41)
			{
				Sound.play(Sound.MDao2, volume);
			}
			else if (skillPaint.id == 45)
			{
				Sound.play(Sound.MDao3, volume);
			}
			else
			{
				Sound.play(Sound.MDao, volume);
			}
			break;
		case 6:
			Sound.play(Sound.MQuat, volume);
			break;
		}
	}
	...
	public void updateCharRun()
	{
		...
		if (!me && currentMovePoint != null && abs(cx - currentMovePoint.xEnd) > num)
		{
			stop();
		}
		...
		try
		{
				...
				if (skillPaint != null && (mobFocus != null || (charFocus != null && isMeCanAttackOtherPlayer(charFocus))))
				{
					...
					if (charFocus != null)
					{
						...
						for (int i = 0; i < GameScr.vCharInMap.size(); i++)
						{
							...
							if (@char.statusMe != 14 && @char.statusMe != 5 && @char.statusMe != 15 && !@char.isInvisible && !@char.Equals(charFocus) && isMeCanAttackOtherPlayer(@char) && charFocus.cx - dx <= @char.cx && @char.cx <= charFocus.cx + dx && charFocus.cy - num <= @char.cy && @char.cy <= charFocus.cy + num && ((cdir == -1 && @char.cx <= cx) || (cdir == 1 && @char.cx >= cx)))
							{
						...
						for (int j = 0; j < GameScr.vMob.size(); j++)
						{
							...
							if (mob.status != 1 && mob.status != 0 && charFocus.cx - dx <= mob.x && mob.x <= charFocus.cx + dx && charFocus.cy - num <= mob.y && mob.y <= charFocus.cy + num && ((cdir == -1 && mob.x <= cx) || (cdir == 1 && mob.x >= cx)))
							{
					...
					if (effPaints.Length > 1)
					{
						...
						for (int num2 = 1; num2 < effPaints.Length; num2++)
						{
							if (effPaints[num2].eMob != null)
							{
								myVector3.addElement(new EPosition(effPaints[num2].eMob.x, effPaints[num2].eMob.y));
							}
							else if (effPaints[num2].eChar != null)
							{
								myVector3.addElement(new EPosition(effPaints[num2].eChar.cx, effPaints[num2].eChar.cy));
							}
							if (num2 > 5)
							{
								break;
			...
			else if (skillPaint != null && (mobFocus != null || charFocus != null))
			{
				...
				if (attMobs != null && attChars != null)
				{
					...
					for (int num3 = 0; num3 < attMobs.Length; num3++)
					{
						effPaints[num3] = new EffectPaint();
						effPaints[num3].effCharPaint = GameScr.efs[skillPaint.effId - 1];
						effPaints[num3].eMob = attMobs[num3];
					}
					for (int num4 = 0; num4 < attChars.Length; num4++)
					{
						effPaints[num4 + attMobs.Length] = new EffectPaint();
						effPaints[num4 + attMobs.Length].effCharPaint = GameScr.efs[skillPaint.effId - 1];
						effPaints[num4 + attMobs.Length].eChar = attChars[num4];
					}
					attMobs = null;
					attChars = null;
				}
				else if (attMobs != null)
				{
					...
					for (int num5 = 0; num5 < attMobs.Length; num5++)
					{
						effPaints[num5] = new EffectPaint();
						effPaints[num5].effCharPaint = GameScr.efs[skillPaint.effId - 1];
						effPaints[num5].eMob = attMobs[num5];
					}
					attMobs = null;
				}
				...
				if (effPaints.Length > 1 && effPaints[0] != null)
				{
					...
					for (int num7 = 1; num7 < effPaints.Length; num7++)
					{
						if (effPaints[num7].eMob != null)
						{
							myVector4.addElement(new EPosition(effPaints[num7].eMob.x, effPaints[num7].eMob.y));
						}
						else if (effPaints[num7].eChar != null)
						{
							myVector4.addElement(new EPosition(effPaints[num7].eChar.cx, effPaints[num7].eChar.cy));
						}
						if (num7 > 5)
						{
							break;
	...
	public bool isHaveWolf()
	{
		...
		return arrItemMounts != null && arrItemMounts[4] != null && arrItemMounts[4].template != null && (arrItemMounts[4].template.id == 443 || arrItemMounts[4].template.id == 523);
	}
	...
	public bool isHaveMoto()
	{
		...
		return arrItemMounts != null && arrItemMounts[4] != null && arrItemMounts[4].template != null && (arrItemMounts[4].template.id == 485 || arrItemMounts[4].template.id == 524);
	}
	...
	public virtual void paint(mGraphics g)
	{
		...
		if (statusMe != 1 && statusMe != 6)
		{
			for (int j = 0; j < vDomsang.size(); j++)
			{
				((Domsang)vDomsang.elementAt(j)).paint(g);
			}
		}
	...
	public void loadFromServer()
	{
		for (int i = 0; i < 26; i++)
		{
			...
			if ((Image)SmallImage.imgNew.get(id + string.Empty) == null)
			{
	...
	public int getFrameRank()
	{
		if (isMoto || isWolf || isNewMount || ID_HORSE > -1)
		{
			if (statusMe == 1 || statusMe == 6)
			{
				return cf + FrameRank * 30;
			}
			return fRun_PP[fRank] + FrameRank * 30;
		}
		return cf + FrameRank * 30;
	}

	public int getFrameName()
	{
		if (isMoto || isWolf || isNewMount || ID_HORSE > -1)
		{
			if (statusMe == 1 || statusMe == 6)
			{
				return cf + FrameName * 30;
			}
			return fRun_PP[fName] + FrameName * 30;
		}
		return cf + FrameName * 30;
	}
	...
	public void paintMatNaBot(mGraphics g, int cx, int cy, int f, DataSkillEff partMatNa)
	{
		...
		if (isNewMount)
		{
			...
			partMatNa.paintBottomEff_new(g, cx + getxs(), cy + num - getDyHorse() + dyMount - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
		else if (isWolf)
		{
			partMatNa.paintBottomEff_new(g, cx + getxs(), cy + num - getDyHorse() - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
		else if (isMoto)
		{
			partMatNa.paintBottomEff_new(g, cx + getxs(), cy + num - getDyHorse() - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		...
		{
			partMatNa.paintBottomEff_new(g, cx + getxs() + dxnewhorse, cy, getFrameMatna() - getys() - getDyHorse(), (cdir != 1) ? 2 : 0);
		}
		else
		{
			partMatNa.paintBottomEff_new(g, cx + getxs(), cy, getFrameMatna() - getys(), (cdir != 1) ? 2 : 0);
		}
	}
	...
	public void paintMatNaTop(mGraphics g, int cx, int cy, int f, DataSkillEff partMatNa)
	{
		...
		if (isNewMount)
		{
			...
			partMatNa.paintTopEff_new(g, cx + getxs(), cy + num - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
		else if (isWolf)
		{
			partMatNa.paintTopEff_new(g, cx + getxs(), cy + num - getDyHorse() - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
		else if (isMoto)
		{
			partMatNa.paintTopEff_new(g, cx + getxs(), cy + num - getDyHorse() - getys(), (statusMe != 1 && statusMe != 6) ? (5 + FrameMatna * 30) : getFrameMatna(), (cdir != 1) ? 2 : 0);
		...
		if (ID_HORSE > -1)
		{
			partMatNa.paintTopEff_new(g, cx + getxs() + dxnewhorse, cy - getys() - getDyHorse(), getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
		else
		{
			partMatNa.paintTopEff_new(g, cx + getxs(), cy - getys(), getFrameMatna(), (cdir != 1) ? 2 : 0);
		}
	}
	...
	public bool isoldHorse()
	{
		...
		return arrItemMounts[4].template.id == 485 || arrItemMounts[4].template.id == 524 || arrItemMounts[4].template.id == 523 || arrItemMounts[4].template.id == 443;
	}
	...
	public void paintPP_Bot(mGraphics g)
	{
		...
		if (isMoto)
		{
			if (arrItemMounts[4].template.id == 485)
			{
				...
				partEff?.paintBottomEff_new(g, cx + getxs(), cy - num2 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
			if (arrItemMounts[4].template.id == 524)
			{
				...
				partEff?.paintBottomEff_new(g, cx + num4 + getxs(), cy - num3 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
		}
		else if (isWolf)
		{
			if (arrItemMounts[4].template.id == 523)
			{
				...
				partEff?.paintBottomEff_new(g, cx + num6 + getxs(), cy - num5 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
			else if (arrItemMounts[4].template.id == 443)
			{
				...
				partEff?.paintBottomEff_new(g, cx + num8 + getxs(), cy - num7 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
		}
		else if (isNewMount)
		{
			...
			partEff?.paintBottomEff_new(g, cx + num10 + getxs(), cy - num9 + dyMount + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
		}
		else
		{
	...
	public void paintPP_Top(mGraphics g)
	{
		...
		if (isMoto)
		{
			if (arrItemMounts[4].template.id == 485)
			{
				...
				partEff?.paintTopEff_new(g, cx + getxs(), cy - num2 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
			if (arrItemMounts[4].template.id == 524)
			{
				int num3 = 12;
				...
				partEff?.paintTopEff_new(g, cx + num4 + getxs(), cy - num3 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
		}
		else if (isWolf)
		{
			if (arrItemMounts[4].template.id == 523)
			{
				...
				partEff?.paintTopEff_new(g, cx + num6 + getxs(), cy - num5 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
			else if (arrItemMounts[4].template.id == 443)
			{
				if ((statusMe == 1 || statusMe == 6) && cf <= 1)
				...
				partEff?.paintTopEff_new(g, cx + num8 + getxs(), cy - num7 + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
			}
		}
		else if (isNewMount)
		{
			...
			partEff?.paintTopEff_new(g, cx + num10 + getxs(), cy - num9 + dyMount + num - getys(), getFramePP(), (cdir != 1) ? 2 : 0);
		}
		else
		{
	...
	private void paintCharWithoutSkill(mGraphics g)
	{
		try
		{
			...
			if (partEff4 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num4 = partEff4.listFrame.size() / 30;
				if (num4 == 0)
				{
					num4 = 1;
				}
				FrameRank = (byte)((FrameRank + 1) % num4);
			}
			DataSkillEff partEff5 = getPartEff(ID_NAME, ishorse: false);
			if (partEff5 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num5 = partEff5.listFrame.size() / 30;
				if (num5 == 0)
				{
					num5 = 1;
				}
				FrameName = (byte)((FrameName + 1) % num5);
			}
			DataSkillEff partEff6 = getPartEff(ID_LEG, ishorse: false);
			if (partEff6 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num6 = partEff6.listFrame.size() / 30;
				if (num6 == 0)
				{
					num6 = 1;
				}
				FrameLeg = (byte)((FrameLeg + 1) % num6);
			}
			DataSkillEff partEff7 = getPartEff(ID_HORSE, ishorse: true);
			if (partEff7 != null && partEff7.isLoadData && GameCanvas.gameTick % 10 == 0)
			{
				int num7 = partEff7.listFrame.size() / partEff7.sequence.Length;
				if (num7 == 0)
				{
					num7 = 1;
				}
				FrameHorse = (byte)((FrameHorse + 1) % num7);
			}
			DataSkillEff partEff8 = getPartEff(ID_Body, ishorse: false);
			if (partEff8 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num8 = partEff8.listFrame.size() / 30;
				if (num8 == 0)
				{
					num8 = 1;
				}
				FrameBody = (byte)((FrameBody + 1) % num8);
			}
			DataSkillEff partEff9 = getPartEff(ID_HAIR, ishorse: false);
			if (partEff9 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num9 = partEff9.listFrame.size() / 30;
				if (num9 == 0)
				{
					num9 = 1;
				}
				FrameHair = (byte)((FrameHair + 1) % num9);
			}
			DataSkillEff partEff10 = getPartEff(ID_PP, ishorse: false);
			if (partEff10 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num10 = partEff10.listFrame.size() / 30;
				if (num10 == 0)
				{
					num10 = 1;
				}
				FramePP = (byte)((FramePP + 1) % num10);
			}
			Part part = GameScr.parts[head];
			Part part2 = GameScr.parts[leg];
			Part part3 = GameScr.parts[body];
			...
			int[] array = ((part.pi[CharInfo[cf][0][0]].id > 4) ? getClassCoat() : null);
			...
			else
			{
				...
				partEff4?.paintBottomEff_new(g, cx, cy - getDyHorse(), getFrameRank(), (cdir != 1) ? 2 : 0);
				partEff5?.paintBottomEff_new(g, cx, cy - getDyHorse(), getFrameName(), (cdir != 1) ? 2 : 0);
				if (partEff3 != null)
				{
					paintMatNaBot(g, cx, cy, 2, partEff3);
				}
				paintPP_Bot(g);
				if (isMoto)
				{
					...
					if (arrItemMounts[4].template.id == 485)
					{
						if (arrItemMounts[4].sys < 2)
						{
							if (statusMe == 1 || statusMe == 6)
							{
								num11 = ((GameCanvas.gameTick % 20 > 12) ? 1 : 0);
								if (partEff10 != null || partEff8 != null || partEff9 != null || partEff != null || partEff3 != null)
								{
									num11 = cf;
								}
							}
							...
							{
								num11 = ((GameCanvas.gameTick % 6 > 3) ? 1 : 0);
							}
							if (statusMe == 3)
							{
								num12 = -5 * cdir;
							...
							if (cdir == 1)
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 1710 : 1709, cx, cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								if (partEff9 != null)
								{
								...
								{
									SmallImage.drawSmallImage(g, getHeadId(), cx + num12 + dxHead * cdir, cy - CharInfo[0][0][2] + part.pi[CharInfo[0][0][0]].dy - 12 + num11 + dyHead, 0, mGraphics.TOP | mGraphics.HCENTER);
								}
								if (partEff8 != null)
								{
								...
								{
									SmallImage.drawSmallImage(g, getBodyPaintId(), cx + dxBody * cdir, cy - SmallImage.getHeight(getLegId()) - 8 + num11 + dyBody, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
							}
							else
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 1710 : 1709, cx, cy, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								if (partEff9 != null)
								{
									paintHair(g, cx, cy - 8, 2);
								...
								{
									SmallImage.drawSmallImage(g, getHeadId(), cx + num12 + dxHead * cdir, cy - CharInfo[0][0][2] + part.pi[CharInfo[0][0][0]].dy - 12 + num11 + dyHead, 2, mGraphics.TOP | mGraphics.HCENTER);
								}
								if (partEff8 != null)
								{
									paintBody(g, cx, cy - 8, 2);
								...
								{
									SmallImage.drawSmallImage(g, getBodyPaintId(), cx + dxBody * cdir, cy - SmallImage.getHeight(getLegId()) - 8 + num11 + dyBody, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
							}
						}
						else
						{
							...
							if (cdir == 1)
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2055 : 2056, cx, cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								else
								{
								...
								else
								{
									...
									{
										SmallImage.drawSmallImage(g, getHeadId(), cx + num12 + dxHead * cdir - 3, cy - 1 - CharInfo[0][0][2] + part.pi[CharInfo[0][0][0]].dy - 12 + num11 + dyHead, 0, mGraphics.TOP | mGraphics.HCENTER);
									}
									if (partEff8 != null)
									{
										paintBody(g, cx, cy - 8, 2);
									...
									{
										SmallImage.drawSmallImage(g, getBodyPaintId(), cx - 3 + dxBody * cdir, cy - 1 - SmallImage.getHeight(getLegId()) - 8 + num11 + dyBody, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
									}
								}
							}
							else
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2055 : 2056, cx, cy, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								else
								{
					...
					else if (arrItemMounts[4].template.id == 524)
					{
						if (arrItemMounts[4].sys < 2)
						{
							...
							if (cdir == 1)
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2065 : 2064, cx, cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								if (partEff9 != null)
								{
									paintHair(g, cx, cy - 8, 2);
							...
							else
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2065 : 2064, cx, cy, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								if (partEff9 != null)
								{
									paintHair(g, cx, cy - 8, 2);
						...
						else if (arrItemMounts[4].sys >= 2 && arrItemMounts[4].sys < 4)
						{
							...
							if (cdir == 1)
							{
								...
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2069 : 2068, cx, cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								else if (statusMe == 2 || statusMe == 10)
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2069 : 2068, cx, cy, 0, mGraphics.BOTTOM | mGraphics.HCENTER);
							...
							else
							{
								...
								if (statusMe == 4)
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2069 : 2068, cx, cy, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
								}
								else if (statusMe == 2 || statusMe == 10)
								{
									SmallImage.drawSmallImage(g, (num11 != 0) ? 2069 : 2068, cx, cy, 2, mGraphics.BOTTOM | mGraphics.HCENTER);
				...
				else
				{
					...
					if (isNewMount)
					{
						...
						paintPP_Top(g);
						partEff5?.paintTopEff_new(g, cx, cy - getDyHorse(), getFrameName(), (cdir != 1) ? 2 : 0);
						partEff4?.paintTopEff_new(g, cx, cy - getDyHorse(), getFrameRank(), (cdir != 1) ? 2 : 0);
						if (partEff != null)
						{
				...
				if (partEff3 != null)
				{
					paintMatNaTop(g, cx, cy, 2, partEff3);
				}
				if (partEff != null)
				{
					paintWeaponTop(g, cx, cy, 2, partEff);
	...
	private int getBodyPaintId()
	{
		...
		if (statusMe == 3)
		{
			CharPartInfo charPartInfo = (CharPartInfo)CharPartInfo.body_jump.get(body + string.Empty);
		...
		if (!isBocdau)
		{
			CharPartInfo charPartInfo3 = (CharPartInfo)CharPartInfo.body_normal.get(body + string.Empty);
			if (charPartInfo3 != null)
			{
				CharPartInfo charPartInfo4 = (CharPartInfo)charPartInfo3.item.get(arrItemMounts[4].template.id + string.Empty);
		...
		CharPartInfo charPartInfo5 = (CharPartInfo)CharPartInfo.body_boc_dau.get(body + string.Empty);
		if (charPartInfo5 != null)
		{
			CharPartInfo charPartInfo6 = (CharPartInfo)charPartInfo5.item.get(arrItemMounts[4].template.id + string.Empty);
			if (charPartInfo6 != null)
			{
				dxBody = charPartInfo6.dx;
				dyBody = charPartInfo6.dy;
				if (part != null)
				{
					return part.pi[frameFix].id;
				}
				return charPartInfo5.idSmall;
			}
		}
		switch (body)
		{
	...
	private int getHeadId()
	{
		...
		if (statusMe == 3)
		{
			CharPartInfo charPartInfo = (CharPartInfo)CharPartInfo.head_jump.get(head + string.Empty);
			if (charPartInfo != null)
			{
				CharPartInfo charPartInfo2 = (CharPartInfo)charPartInfo.item.get(arrItemMounts[4].template.id + string.Empty);
				if (charPartInfo2 != null)
		...
		if (!isBocdau)
		{
			CharPartInfo charPartInfo3 = (CharPartInfo)CharPartInfo.head_normal.get(head + string.Empty);
			if (charPartInfo3 != null)
			{
				CharPartInfo charPartInfo4 = (CharPartInfo)charPartInfo3.item.get(arrItemMounts[4].template.id + string.Empty);
				if (charPartInfo4 != null)
		...
		CharPartInfo charPartInfo5 = (CharPartInfo)CharPartInfo.head_boc_dau.get(head + string.Empty);
		if (charPartInfo5 != null)
		{
			CharPartInfo charPartInfo6 = (CharPartInfo)charPartInfo5.item.get(arrItemMounts[4].template.id + string.Empty);
			if (charPartInfo6 != null)
			{
				dxHead = charPartInfo6.dx;
				dyHead = charPartInfo6.dy;
				if (part != null)
				{
					return part.pi[frameFix].id;
				}
	...
	public static bool isCharInScreen(Char c)
	{
		...
		if (c.statusMe != 15 && !c.isInvisible && cmx <= c.cx && c.cx <= num && num2 <= c.cy)
		{
			return c.cy <= num3;
		}
		return false;
	...
	public void moveTo(int toX, int toY)
	{
		...
		int xEnd = cx + num;
		int yEnd = cy + num2;
		vMovePoints.addElement(new MovePoint(xEnd, yEnd, act, dir));
		statusMe = 6;
	}
	...
	public void findNextFocusByKey()
	{
		...
		if (nClass.classId == 0 || nClass.classId == 1 || nClass.classId == 3)
		{
			_ = 1;
		}
		else
		...
		if (TileMap.typeMap == TileMap.MAP_CHIENTRUONG || TileMap.mapID == 111)
		{
			if (TileMap.mapID == 98 || TileMap.mapID == 104)
			{
				...
				{
					Char @char = (Char)GameScr.vCharInMap.elementAt(i);
					if (!@char.isNhanbanz() && @char.statusMe != 15 && !@char.isInvisible && num2 <= @char.cx && @char.cx <= num3 && num4 <= @char.cy && @char.cy <= num5)
					{
			...
			else
			{
				for (int j = 0; j < GameScr.vCharInMap.size(); j++)
				{
					...
					if (char2.isNhanbanz() || char2.statusMe == 15 || char2.isInvisible || num2 > char2.cx || char2.cx > num3 || num4 > char2.cy || char2.cy > num5)
					{
						continue;
					}
					if (TileMap.mapID != 111)
			...
			for (int l = 0; l < GameScr.vMob.size(); l++)
			{
				...
				if ((mob.templateId != 97 || getMyChar().cTypePk != PK_PHE1) && (mob.templateId != 98 || getMyChar().cTypePk != PK_PHE1) && (mob.templateId != 167 || getMyChar().cTypePk != PK_PHE1) && (mob.templateId != 96 || getMyChar().cTypePk != PK_PHE2) && (mob.templateId != 99 || getMyChar().cTypePk != PK_PHE2) && (mob.templateId != 166 || getMyChar().cTypePk != PK_PHE2) && (mob.templateId != 200 || getMyChar().cTypePk != PK_PHE1) && (mob.templateId != 199 || getMyChar().cTypePk != PK_PHE2) && (mob.templateId != 198 || getMyChar().cTypePk != PK_PHE3) && (mob.templateId != 202 || mob.status != 8 || mob.isPaint()) && mob.status != 1 && mob.status != 0 && num2 <= mob.x && mob.x <= num3 && num4 <= mob.y && mob.y <= num5)
				{
			...
			if (focus.size() > 0)
			{
				...
				if (focus.elementAt(num) is Char)
				{
					mobFocus = null;
					deFocusNPC();
					charFocus = (Char)focus.elementAt(num);
					itemFocus = null;
					isManualFocus = true;
				}
				else if (focus.elementAt(num) is Npc)
				{
					mobFocus = null;
					deFocusNPC();
					npcFocus = (Npc)focus.elementAt(num);
					charFocus = null;
					itemFocus = null;
					isManualFocus = true;
				}
				else if (focus.elementAt(num) is Mob)
				{
					mobFocus = (Mob)focus.elementAt(num);
					deFocusNPC();
					charFocus = null;
					itemFocus = null;
					isManualFocus = true;
				}
				else if (focus.elementAt(num) is ItemMap)
				{
					mobFocus = null;
					deFocusNPC();
					charFocus = null;
					itemFocus = (ItemMap)focus.elementAt(num);
					isManualFocus = true;
				}
			}
			else
		...
		for (int n = 0; n < GameScr.vItemMap.size(); n++)
		{
			...
			if (num2 <= itemMap2.x && itemMap2.x <= num3 && num4 <= itemMap2.y && itemMap2.y <= num5)
			{
		...
		for (int num6 = 0; num6 < GameScr.vMob.size(); num6++)
		{
			Mob mob2 = (Mob)GameScr.vMob.elementAt(num6);
			if ((mob2.templateId != 97 || getMyChar().cTypePk != PK_PHE1) && (mob2.templateId != 96 || getMyChar().cTypePk != PK_PHE2) && (mob2.templateId != 98 || getMyChar().cTypePk != PK_PHE1) && (mob2.templateId != 167 || getMyChar().cTypePk != PK_PHE1) && (mob2.templateId != 99 || getMyChar().cTypePk != PK_PHE2) && (mob2.templateId != 166 || getMyChar().cTypePk != PK_PHE2) && (mob2.templateId != 200 || getMyChar().cTypePk != PK_PHE1) && (mob2.templateId != 199 || getMyChar().cTypePk != PK_PHE2) && (mob2.templateId != 198 || getMyChar().cTypePk != PK_PHE3) && (mob2.templateId != 202 || mob2.status != 8 || mob2.isPaint()) && mob2.status != 1 && mob2.status != 0 && num2 <= mob2.x && mob2.x <= num3 && num4 <= mob2.y && mob2.y <= num5)
			{
		...
		for (int num7 = 0; num7 < GameScr.vNpc.size(); num7++)
		{
			...
			if (npc2.statusMe != 15 && num2 <= npc2.cx && npc2.cx <= num3 && num4 <= npc2.cy && npc2.cy <= num5)
			{
		...
		for (int num8 = 0; num8 < GameScr.vCharInMap.size(); num8++)
		{
			Char char3 = (Char)GameScr.vCharInMap.elementAt(num8);
			if (!char3.isNhanbanz() && char3.statusMe != 15 && !char3.isInvisible && num2 <= char3.cx && char3.cx <= num3 && num4 <= char3.cy && char3.cy <= num5)
			{
		...
		if (focus.size() > 0)
		{
			...
			else if (focus.elementAt(num) is Char)
			{
				mobFocus = null;
				deFocusNPC();
				charFocus = (Char)focus.elementAt(num);
	...
	public bool isHaveNewMount()
	{
		...
		return arrItemMounts != null && arrItemMounts[4] != null && arrItemMounts[4].template != null && arrItemMounts[4].template.id != 443 && arrItemMounts[4].template.id != 523 && arrItemMounts[4].template.id != 485 && arrItemMounts[4].template.id != 524;
	}
	...
	public static bool Move(int cxSend, int cySend)
	{
		...
		if (cxSend > num)
		{
			do
			{
				Service.gI().charMove(num, TileMap.pMove(num, n));
			}
			while ((num += 50) < cxSend);
		}
		else
		...
		Service.gI().charMove(cxSend, cySend);
	...
	public static void PickNpc(int npcID, int menuID, int optionID)
	{
		...
		if ((npc = FindNpc(npcID)) != null)
		{
			...
			Service.gI().menu(npcID, menuID, optionID);
		}
	}
