...
public class GameScr : mScreen, IChatable, IActionListener
{
	...
	protected void doSetOnScreenSkill(SkillTemplate skillTemplate)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			myVector2.addElement(new Command(mResources.CELLS + " " + (i + 1), 11120, p));
			if (flag)
			{
				break;
			}
	...
	public static long[] getLevelExp(long exp)
	{
		long num = exp;
		int i;
		for (i = 0; i < exps.Length && num >= exps[i]; i++)
		{
			num -= exps[i];
		}
		return new long[2] { i, num };
	}
	...
	private void initCommand()
	{
		...
		cmdBagViewModItem = new Command("Xem", 110020);
	...
	public bool checkExistFriend(string strName)
	{
		...
		{
			if (((Friend)vFriend.elementAt(i)).friendName.Equals(strName))
			{
				return false;
			}
		}
	...
	{
		if ((Char.getMyChar().myskill == null || Char.getMyChar().cMP >= Char.getMyChar().myskill.manaUse) && Char.getMyChar().myskill != null && (Char.getMyChar().myskill.template.maxPoint <= 0 || Char.getMyChar().myskill.point != 0))
		{
			return Char.getMyChar().arrItemBody[1] != null;
		}
		return false;
	}
	...
	private bool isMobSameParty()
	{
		if (Char.getMyChar().mobFocus != null)
		{
			...
			{
				if (Char.getMyChar().mobFocus.getTemplate().mobTemplateId == 143)
				{
					return Char.getMyChar().cTypePk == Char.PK_PHE3;
				}
				return false;
			}
			return true;
	...
	private void updateKeyTouchControl()
	{
		...
		{
			selectedIndexSkill = (GameCanvas.isTouchControl ? ((GameCanvas.pxLast - (xSkill + xS[0])) / 30) : ((GameCanvas.pyLast - (ySkill + yS[0])) / 25));
			if (indexSelect < 0)
			{
				indexSelect = 0;
			}
	...
	private void paintFoodEffect(mGraphics g)
	{
		try
		{
			_ = mSystem.getCurrentTimeMillis() / 1000;
			int num = 5;
			if (GameCanvas.isTouch && GameCanvas.isTouchControl)
			{
				num = 45 + Info.hI;
			...
			for (int i = 0; i < vParty.size(); i++)
			{
				...
				if (party.c != null)
				{
					mFont.tahoma_7_white.drawString(g, party.name + "(" + party.c.clevel + ")", GameCanvas.w - 14, num += 18, 1, mFont.tahoma_7_grey);
					party.c.paintHp(g, GameCanvas.w - 41, num + 12);
					SmallImage.drawSmallImage(g, party.iconId, GameCanvas.w - 7, num + 9, 0, mGraphics.VCENTER | mGraphics.HCENTER);
				}
				else
				{
					num += 16;
					mFont.tahoma_7_green.drawString(g, party.name, GameCanvas.w - 14, num + 5, 1, mFont.tahoma_7_grey);
					SmallImage.drawSmallImage(g, party.iconId, GameCanvas.w - 7, num + 11, 0, mGraphics.VCENTER | mGraphics.HCENTER);
				}
			}
		}
		catch (Exception)
	...
	private void paintDebugInfo(mGraphics g)
	{
		...
		try
		{
			...
			if (Char.getMyChar().isHuman || Char.getMyChar().isNhanban)
			{
				xM[0] = num3;
				yM[0] = num2 + 12;
				string st4 = "Map: " + TileMap.mapID + " | Zone: " + TileMap.zoneID + " | X: " + Char.getMyChar().cx + " | Y: " + Char.getMyChar().cy + " | Zoom: " + mGraphics.zoomLevel;
				if (GameCanvas.gameTick % 50 > 4)
				{
			...
			{
				string st5 = string.Empty;
				xM[0] = num3;
				yM[0] = num2 + 12;
				if (ChatManager.isMessageClan && ChatManager.isMessagePt)
				{
					st5 = mResources.CLANPT_MESSAGE[0];
				}
				else if (ChatManager.isMessageClan)
				{
					st5 = mResources.CLANPT_MESSAGE[1];
				}
				else if (ChatManager.isMessagePt)
				{
					st5 = mResources.CLANPT_MESSAGE[2];
				}
				if (GameCanvas.gameTick % 10 > 7)
				{
					mFont.tahoma_7_red.drawString(g, st5, num3, num2 += 12, 0, mFont.tahoma_7_grey);
				}
				else
				{
					mFont.tahoma_7_yellow.drawString(g, st5, num3, num2 += 12, 0, mFont.tahoma_7_grey);
				}
			}
			...
			{
				num = 4;
				string text3 = Char.getMyChar().mobFocus.getTemplate().name + " lv" + Char.getMyChar().mobFocus.level.ToString() + " tid: " + Char.getMyChar().mobFocus.templateId;
	...
	public bool isPcLevelSmall()
	{
		if (!Main.isPC)
		{
			return Char.getMyChar().ctaskId > 1;
		}
		return true;
	}
	...
	private void paintInfoBar(mGraphics g)
	{
		if (GameCanvas.isTouch)
		{
			...
			long num2 = ((Char.getMyChar().cExpDown <= 0) ? (Char.getMyChar().cExpR * 10000 / exps[Char.getMyChar().clevel]) : (Char.getMyChar().cExpDown * 10000 / exps[Char.getMyChar().clevel]));
	...
	public void paintTrangbi(mGraphics g)
	{
		...
		{
			currentCharViewInfo.paintWeaponUIBot(g, gW2, gH2 - num19, f);
		}
		if (currentCharViewInfo.ID_PP > -1)
		{
			currentCharViewInfo.paintPPUIBot(g, gW2, gH2 - num19, f);
		}
		...
		{
			currentCharViewInfo.paintLegUI(g, gW2, gH2 - num19, f);
		}
		else
		{
		...
		{
			currentCharViewInfo.paintBodyUI(g, gW2, gH2 - num19, f);
		}
		else
		{
		...
		{
			currentCharViewInfo.paintHeadUI(g, gW2, gH2 - num19, f);
		}
		else
		{
		...
		if (currentCharViewInfo.ID_PP > -1)
		{
			currentCharViewInfo.paintPPUITop(g, gW2, gH2 - num19, f);
		}
		if (currentCharViewInfo.ID_WEA_PONE > -1)
		{
			currentCharViewInfo.paintWeaponUITop(g, gW2, gH2 - num19, f);
		}
	...
	public void paintClanInfo(mGraphics g)
	{
		...
		else if (indexMenu == 2)
		{
			...
			Item[] items = ((Char.clan != null && Char.clan.items != null) ? Char.clan.items : new Item[30]);
			paintBagItem(g, items);
		}
		else if (indexMenu == 3)
		{
	...
	private void updateKeyUIInforMeTouch()
	{
		...
		else if (indexMenu == 4)
		{
			if (GameCanvas.isPointerJustRelease)
			{
				...
				{
					int num3 = (GameCanvas.pxLast - (popupX + 4)) / indexSize;
					indexSelect = 10 + num3;
					left = (center = null);
					setLCR();
					Sound.play(Sound.MBClick, 0.5f);
		...
		else
		{
			...
			if (indexBijuuMenu == 1)
			{
				...
				{
					int num4 = (GameCanvas.py - (popupY + 52)) / 32;
					num4++;
					if (num4 == indexTiemNang)
					{
						actBijuuMenuPoint();
					}
					indexTitle = num4;
	...
	public void updateKeyAlert()
	{
		...
		else
		{
			...
			fnick = text2.Substring(startIndex + 1, num - 1);
	...
	private void setFriendCommand()
	{
		...
		{
			_ = (Friend)vFriend.elementAt(indexRow);
	...
	{
		if (GameCanvas.isTouch && !Char.ischangingMap && !isPaintZone && !isPaintAuto && !ChatTextField.gI().isShow && (center != cmdDead || !GameCanvas.isTouchControlLargeScreen) && GameCanvas.currentDialog == null && ChatPopup.currentMultilineChatPopup == null && !GameCanvas.menu.showMenu)
		{
			return isPaintPopup();
		}
		return true;
	...
	{
		if (!isPaintRankedList && (!isPaintInfoMe || indexMenu != 6 || indexBijuuMenu == 0) && (!isPaintInfoMe || indexMenu <= 0 || indexMenu >= 5) && !isPaintAuto && !isPaintZone && (!isPaintClan || (indexMenu != 0 && indexMenu != 1 && indexMenu != 3 && indexMenu != 4)) && !isPaintCharInMap && !isPaintTeam && !isPaintFindTeam && !isPaintFriend && !isPaintList && !isPaintEnemies && !isPaintTask && !isPaintMessage && !isPaintAlert)
		{
			return isPaintLucky_Draw;
		}
		return true;
	}
	...
	private void updateKeyBuyItemUITouch()
	{
		...
		else if (isPaintLuckySpin)
		{
			...
			{
				indexSelect = (GameCanvas.pxLast - xstart) / num + (GameCanvas.pyLast - ystart) / num * 3;
				indexTitle = 1;
				updateCommandForUI();
				doviewCardsInfo();
			}
	...
	private bool isCanLuyenThach()
	{
		...
		if (num < 9 && num2 < 9 && (num4 < 10 || num < 3 || num3 != 1))
		{
			if (num4 >= 11 && num2 >= 3)
			{
				return num3 == 1;
	...
	public void updateCommandForUI()
	{
		...
		else if (isPaintQuanNam)
		{
			if (indexMenu == 0)
			{
				Item item = getItemFocus(26);
				if (item != null)
				{
					left = new Command(mResources.BUY, 11092, item);
					if (!GameCanvas.isTouchControlLargeScreen)
					{
						center = new Command((!GameCanvas.isTouch) ? string.Empty : mResources.VIEW, 11088, item);
					}
					else
					{
						updateItemInfo(26, item);
					}
				}
			}
			else if (indexMenu == 1 && Char.getMyChar().arrItemBag[indexSelect] != null)
		...
		else if (isPaintQuanNu)
		{
			if (indexMenu == 0)
			{
				Item item2 = getItemFocus(27);
				if (item2 != null)
				{
					left = new Command(mResources.BUY, 11092, item2);
					if (!GameCanvas.isTouchControlLargeScreen)
					{
		...
		else if (isPaintGiayNam)
		{
			if (indexMenu == 0)
			{
				Item item3 = getItemFocus(28);
				if (item3 != null)
				{
					left = new Command(mResources.BUY, 11092, item3);
					if (!GameCanvas.isTouchControlLargeScreen)
					{
		...
		else if (isPaintGiayNu)
		{
			if (indexMenu == 0)
			{
				Item item4 = getItemFocus(29);
				if (item4 != null)
				{
					left = new Command(mResources.BUY, 11092, item4);
					if (!GameCanvas.isTouchControlLargeScreen)
					{
		...
		else if (isPaintLien)
		{
			if (indexMenu == 0)
			{
				Item item5 = getItemFocus(16);
				if (item5 != null)
				{
					left = new Command(mResources.BUY, 11092, item5);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(16, item5);
					}
					else
					{
		...
		else if (isPaintNhan)
		{
			if (indexMenu == 0)
			{
				Item item6 = getItemFocus(17);
				if (item6 != null)
				{
					left = new Command(mResources.BUY, 11092, item6);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(17, item6);
					}
					else
					{
		...
		else if (isPaintNgocBoi)
		{
			if (indexMenu == 0)
			{
				Item item7 = getItemFocus(18);
				if (item7 != null)
				{
					left = new Command(mResources.BUY, 11092, item7);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(18, item7);
					}
					else
					{
		...
		else if (isPaintPhu)
		{
			if (indexMenu == 0)
			{
				Item item8 = getItemFocus(19);
				if (item8 != null)
				{
					left = new Command(mResources.BUY, 11092, item8);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(19, item8);
					}
					else
					{
		...
		else if (isPaintWeapon)
		{
			if (indexMenu == 0)
			{
				Item item9 = getItemFocus(2);
				if (item9 != null)
				{
					left = new Command(mResources.BUY, 11092, item9);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(2, item9);
					}
					else
					{
		...
		else if (isPaintStack)
		{
			if (indexMenu == 0)
			{
				Item item10 = getItemFocus(6);
				if (item10 != null)
				{
					left = new Command(mResources.BUY, 11092, item10);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(6, item10);
					}
					else
					{
		...
		else if (isPaintStackLock)
		{
			if (indexMenu == 0)
			{
				Item item11 = getItemFocus(7);
				if (item11 != null)
				{
					left = new Command(mResources.BUY, 11092, item11);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(7, item11);
					}
					else
					{
		...
		else if (isPaintGrocery)
		{
			if (indexMenu == 0)
			{
				Item item12 = getItemFocus(8);
				if (item12 != null)
				{
					left = new Command(mResources.BUY, 11092, item12);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(8, item12);
					}
					else
					{
		...
		else if (isPaintGroceryLock)
		{
			if (indexMenu == 0)
			{
				Item item13 = getItemFocus(9);
				if (item13 != null)
				{
					left = new Command(mResources.BUY, 11092, item13);
					if (GameCanvas.isTouchControlLargeScreen)
					{
						updateItemInfo(9, item13);
					}
					else
					{
		...
		else
		{
			...
			if (isPaintSplit)
			{
				if (indexMenu == 0)
				{
					...
					{
						Item item16 = arrItemSplit[indexSelect];
						if (item16 != null)
						{
							if (GameCanvas.isTouchControlLargeScreen)
							{
								updateItemInfo(3, item16);
							}
							else
							{
								center = new Command((!GameCanvas.isTouch) ? string.Empty : mResources.VIEW, 11104, item16);
							}
						}
						else
						{
			...
			else if (isPaintTinhluyen)
			{
				try
				{
					if (indexMenu == 0)
					{
						...
						{
							Item item17 = arrItemSplit[indexSelect];
							if (item17 != null)
							{
								if (GameCanvas.isTouchControlLargeScreen)
								{
									updateItemInfo(3, item17);
								}
								else
								{
									center = new Command((!GameCanvas.isTouch) ? string.Empty : mResources.VIEW, 11104, item17);
								}
								left = new Command(mResources.MOVEOUT, 1605);
							}
							else
			...
			else if (isPaintDichChuyen)
			{
				if (indexMenu == 0)
				{
					...
					{
						Item item18 = arrItemSplit[indexSelect];
						if (item18 != null)
						{
							if (GameCanvas.isTouchControlLargeScreen)
							{
								updateItemInfo(3, item18);
							}
							else
							{
								center = new Command((!GameCanvas.isTouch) ? string.Empty : mResources.VIEW, 11104, item18);
							}
						}
						else
						{
			...
			else if (isPaintLuyenThach)
			{
				if (indexMenu == 0)
				{
					...
					{
						Item item20 = arrItemUpPeal[l];
						if (item20 != null)
						{
							if (item20.template.id == 455)
							{
								num4++;
							}
							else if (item20.template.id == 456)
							{
								num5++;
							}
							else if (item20.template.type == 26)
							{
								num7 = item20.template.id;
								num6++;
							}
						}
				...
				if (indexMenu == 0)
				{
					if (indexTitle == 1)
					{
						if (arrItemTradeMe[indexSelect] != null)
						{
							...
							if (GameCanvas.isTouchControlLargeScreen)
							{
								Item item22 = arrItemTradeMe[indexSelect];
								updateItemInfo(3, item22);
							}
							else
							{
					...
					if (indexTitle == 2)
					{
						if (arrItemTradeOrder[indexSelect] != null)
						{
							if (GameCanvas.isTouchControlLargeScreen)
							{
								Item item23 = arrItemTradeOrder[indexSelect];
								updateItemInfo(30, item23);
							}
							else
							{
	...
	public void paintKyNang(mGraphics g)
	{
		...
		else
		{
			...
			else
			{
				...
				paintSkillOption(g, skill);
				int l;
				for (l = 0; l < skillTemplate.skills.Length; l++)
				{
					if (skillTemplate.skills[l].Equals(skill))
					{
						l++;
						break;
					}
				}
				mFont.tahoma_7_red.drawString(g, mResources.replace(mResources.LEVELNEXT, string.Empty + skillTemplate.skills[l].point), xstart + 5, yPaint += 12, 0);
				paintSkillInfo(g, skillTemplate.skills[l]);
				indexRowMax++;
				paintSkillOption(g, skillTemplate.skills[l]);
			}
		}
		...
		try
		{
			...
			if (itemUpGrade != null)
			{
				...
				else
				{
					...
					{
						int num6 = num4 * 100 / upClothe[itemUpGrade.upgrade];
						if (num6 > maxPercents[itemUpGrade.upgrade])
						{
							num6 = maxPercents[itemUpGrade.upgrade];
						}
						if (isPaintUpGradeGold)
						{
							num6 = (int)((double)num6 * 1.5);
							num5 = goldUps[itemUpGrade.upgrade];
						}
						if (Main.isPC)
						{
							...
							if (num5 > Char.getMyChar().luong)
							{
								mFont.tahoma_7_red.drawString(g, mResources.replace(mResources.GOLDNEED, num5 + string.Empty), xstart, ystart + rows * indexSize + 17, mFont.LEFT);
							}
							else
							{
								mFont.tahoma_7_yellow.drawString(g, mResources.replace(mResources.GOLDNEED, num5 + string.Empty), xstart, ystart + rows * indexSize + 17, mFont.LEFT);
							}
							mFont.tahoma_7_yellow.drawString(g, mResources.PERCENT + ": " + num6 + "%", xstart, ystart + rows * indexSize + 29, mFont.LEFT);
						}
						else
						{
							...
							if (num5 > Char.getMyChar().luong)
							{
								array[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							else
							{
								array[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							array[2] = mResources.PERCENT + ": " + num6 + "%";
							paintInfoHelpCheNgoc1(g, array, 0);
						}
					}
					else if (itemUpGrade.isTypeAdorn())
					{
						int num7 = num4 * 100 / upAdorn[itemUpGrade.upgrade];
						if (num7 > maxPercents[itemUpGrade.upgrade])
						{
							num7 = maxPercents[itemUpGrade.upgrade];
						}
						if (isPaintUpGradeGold)
						{
						...
						if (Main.isPC)
						{
							...
							mFont.tahoma_7_yellow.drawString(g, mResources.PERCENT + ": " + num7 + "%", xstart, ystart + rows * indexSize + 29, mFont.LEFT);
						}
						else
						{
							...
							if (num5 > Char.getMyChar().luong)
							{
								array2[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							else
							{
								array2[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							array2[2] = mResources.PERCENT + ": " + num7 + "%";
							paintInfoHelpCheNgoc1(g, array2, 0);
						}
					}
					else if (itemUpGrade.isTypeWeapon())
					{
						int num8 = num4 * 100 / upWeapon[itemUpGrade.upgrade];
						if (num8 > maxPercents[itemUpGrade.upgrade])
						{
							num8 = maxPercents[itemUpGrade.upgrade];
						}
						if (isPaintUpGradeGold)
						{
						...
						if (Main.isPC)
						{
							...
							mFont.tahoma_7_yellow.drawString(g, mResources.PERCENT + ": " + num8 + "%", xstart, ystart + rows * indexSize + 29, mFont.LEFT);
						}
						else
						{
							...
							if (num5 > Char.getMyChar().luong)
							{
								array3[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							else
							{
								array3[1] = mResources.replace(mResources.GOLDNEED, num5 + string.Empty);
							}
							array3[2] = mResources.PERCENT + ": " + num8 + "%";
			...
			if (indexTitle == 2)
			{
				int num9 = indexSelect / columns;
				int num10 = indexSelect - num9 * columns;
				g.setColor(16777215);
			...
			for (int m = 0; m < arrItemUpGrade.Length; m++)
			{
				...
				{
					int num11 = m / columns;
					int num12 = m - num11 * columns;
					if (!item.isLock)
					{
						g.setColor(12083);
						g.fillRect(xstart + num12 * indexSize + 1, ystart + num11 * indexSize + 1, indexSize - 1, indexSize - 1);
					}
					SmallImage.drawSmallImage(g, item.template.iconID, xstart + num12 * indexSize + indexSize / 2, ystart + num11 * indexSize + indexSize / 2, 0, mGraphics.VCENTER | mGraphics.HCENTER);
				}
			}
			if (effUpok == null)
			{
	...
	public sbyte getTaskMapId()
	{
		...
		sbyte b = ((Char.getMyChar().taskMaint != null) ? mapTasks[Char.getMyChar().ctaskId][Char.getMyChar().taskMaint.index + 1] : mapTasks[Char.getMyChar().ctaskId][0]);
		if (b == -1)
		{
	...
	public sbyte getTaskNpcId()
	{
		try
		{
			...
			sbyte b = ((Char.getMyChar().taskMaint != null) ? tasks[Char.getMyChar().ctaskId][Char.getMyChar().taskMaint.index + 1] : tasks[Char.getMyChar().ctaskId][0]);
			if (b == -1)
			{
				if (Char.getMyChar().nClass.classId == 0 && Char.getMyChar().ctaskId == 9)
				{
	...
	public void paintItemInfo(mGraphics g)
	{
		...
		if (item.expires != 0L && item.options != null && item.options.size() > 0)
		{
			...
			{
				num = ((ItemOption)item.options.elementAt(i)).getOptionString().Length * 5;
		...
		{
			if (mFont2.splitFontArray(item.template.name + ((item.upgrade <= 0) ? string.Empty : (" +" + item.upgrade)), inforW - ((!GameCanvas.isTouch || GameCanvas.w < 320) ? 10 : 20)).Length > 1)
			{
				yPaint -= 12;
			}
		...
		if (item.expires != 0L && item.options != null && item.options.size() > 0)
		{
			...
			{
				ItemOption itemOption = (ItemOption)item.options.elementAt(num13);
				if (!flag && itemOption.optionTemplate.type == 2)
				{
					flag = true;
				...
				if (!flag2 && itemOption.optionTemplate.type > 2 && itemOption.optionTemplate.type < 8)
				{
					flag2 = true;
				...
				if (itemOption.optionTemplate.id == 65)
				{
					paintMultiLine(g, mFont.tahoma_7_blue, (item.template.id != 485) ? itemOption.getOptionString() : NinjaUtil.replace(itemOption.getOptionString(), mResources.EXP, mResources.MACHINE), inforX + 8, yPaint += 12, mFont.LEFT);
				}
				else if (itemOption.optionTemplate.id == 66)
				{
					paintMultiLine(g, mFont.tahoma_7_blue1, (item.template.id != 485) ? itemOption.getOptionString() : NinjaUtil.replace(itemOption.getOptionString(), mResources.VITALITY, mResources.POWER), inforX + 8, yPaint += 12, mFont.LEFT);
				}
				else if (itemOption.optionTemplate.type == 0)
				{
					paintMultiLine(g, mFont.tahoma_7_blue1, (!item.isTypeUIShopView()) ? itemOption.getOptionString() : itemOption.getOptionShopString(), inforX + 8, yPaint += 12, mFont.LEFT);
				}
				else if (itemOption.optionTemplate.type == 1)
				{
					paintMultiLine(g, mFont.tahoma_7_green, (!item.isTypeUIShopView()) ? itemOption.getOptionString() : itemOption.getOptionShopString(), inforX + 8, yPaint += 12, mFont.LEFT);
				}
				else if (itemOption.optionTemplate.type == 8)
				{
					if (itemOption.optionTemplate.id == 85)
					{
						paintMultiLine(g, mFont.tahoma_7_yellow, (!item.isTypeUIShopView()) ? itemOption.getOptionShopString1() : itemOption.getOptionShopString(), inforX + 8, yPaint += 12, mFont.LEFT);
					}
					else
					{
						paintMultiLine(g, mFont.tahoma_7b_blue, (!item.isTypeUIShopView()) ? itemOption.getOptionShopString() : itemOption.getOptionShopString(), inforX + 8, yPaint += 12, mFont.LEFT);
					}
				}
				...
				{
					paintMultiLine(g, mFont.tahoma_7_grey, (!item.isTypeUIShopView()) ? itemOption.getOptionString() : itemOption.getOptionShopString(), inforX + 8, yPaint += 12, mFont.LEFT);
				}
				indexRowMax++;
			}
		}
	...
	public void actionPerform(int idAction, object p)
	{
		...
		{
			Npc npc2 = (Npc)p;
			Service.gI().getTask(npc2.template.npcTemplateId, GameCanvas.menu.menuSelectedItem, -1);
			break;
		}
		case 11058:
		{
	...
	private void actDichChuyenTrangBiLeft()
	{
		try
		{
			...
			else if (Char.getMyChar().arrItemBag[indexSelect].upgrade > 11)
			{
				...
				{
					Item obj = Char.getMyChar().arrItemBag[indexSelect];
					Char.getMyChar().arrItemBag[indexSelect] = null;
					Char.getMyChar().arrItemBag[itemSplit.indexUI] = itemSplit;
					itemSplit = obj;
				}
	...
	private void doviewCardsInfo()
	{
		if (arrItemSprin != null)
		{
			updateItemInfo(38, new Item
			{
				template = ItemTemplates.get(arrItemSprin[indexSelect])
			});
		}
	}
	...
	public void actRemoveWaitAcceptFriend(string strFWname)
	{
		...
		{
			if (((Friend)vFriendWait.elementAt(j)).friendName.Equals(strFWname))
			{
	...
	private void actSplitItemBag()
	{
		try
		{
			...
			else if (Char.getMyChar().arrItemBag[indexSelect].isTypeBody() && Char.getMyChar().arrItemBag[indexSelect].upgrade > 0)
			{
				...
				{
					Item obj2 = Char.getMyChar().arrItemBag[indexSelect];
					Char.getMyChar().arrItemBag[indexSelect] = null;
					Char.getMyChar().arrItemBag[itemSplit.indexUI] = itemSplit;
					itemSplit = obj2;
				}
	...
	private void actBagSelectItem()
	{
		...
		myVector.addElement(cmdBagViewModItem);
	...
	public void paint_exp_luyenngoc_bar(mGraphics g, int x, int y)
	{
		...
		if (itemSplit.options != null)
		{
			...
			{
				ItemOption itemOption = (ItemOption)itemSplit.options.elementAt(i);
		...
		int num5 = 5;
		g.setColor(0);
		g.fillRect(x, y - num5, num4, num5);
		int num6 = num2 * num4 / num;
		if (num6 <= 0)
		{
			num6 = 0;
		}
		...
		g.setColor(65280);
		g.fillRect(x, y - num5, num6, num5);
		int num7 = num3 * num4 / num;
		if (num7 >= num4 - num6)
		{
			num7 = num4 - num6;
		}
		g.setColor(430630);
		g.fillRect(x + num6, y - num5, num7, num5);
		mFont.tahoma_7_yellow.drawString(g, num3 + num2 + "/" + num, x + num4 / 2, y - num5 - 15, 2);
	}
	...
	public void paintKhamNgoc(mGraphics g, string[] titles)
	{
		try
		{
			...
			if (itemUpGrade != null && itemSplit != null)
			{
				...
				int num9 = num8 * 100 / upWeapon[itemSplit.upgrade];
				...
				if (itemSplit.options != null)
				{
					...
					{
						ItemOption itemOption = (ItemOption)itemSplit.options.elementAt(m);
